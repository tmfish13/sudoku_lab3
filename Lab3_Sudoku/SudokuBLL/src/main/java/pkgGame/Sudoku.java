...
public class Sudoku extends LatinSquare {
	...
	public void setRegion(int region) {
		...
		int row = (int) Math.floor(region / iSqrtSize) * iSqrtSize;
		int col = (int) (region - row) * iSqrtSize;
			
		int count = 1;
			
		for(int x = row; x < row+iSqrtSize; x++) {
			for(int y = col; y < col+iSqrtSize; y++) {
				this.getLatinSquare()[x][y] = count;
				count++;
			}
		}
	...
	public void PrintPuzzle() {
		for(int x = 0; x < iSize; x++) {
			for(int y = 0; y < iSize; y++) {
				System.out.print(this.getLatinSquare()[x][y] + " ");
			}
			System.out.println();
		}
	}
	...
	private void shuffleRegion(int number) {
		int[] reg = getRegion(number);
		shuffleArray(reg);
		
		int row = (int) Math.floor(number / iSqrtSize) * iSqrtSize;
		int col = (int) (number - row) * iSqrtSize;
			
		int list_count = 0;
			
		for(int x = row; x < row+iSqrtSize; x++) {
			for(int y = col; y < col+iSqrtSize; y++) {
				this.getLatinSquare()[x][y] = reg[list_count];
				list_count++;
			}
		}
	}
	...
	private void fillDiagonalRegions() {
		...
		for (int count = 0; count < iSize; count += (iSqrtSize + 1)) {
			setRegion(count);
			shuffleRegion(count);	
		}
